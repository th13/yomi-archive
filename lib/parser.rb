...
# use ve to parse sentence, stores words in database and makes connections
...
def Parser.parse_sentence(jpn_sentence)
 ...
 jpn_words.each do |word|
   word = Word.find_or_create_by(jpn: word)
   Keyword.create!(sentence_id: jpn_sentence[:id], word_id: word[:id])
 end
